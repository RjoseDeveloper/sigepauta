...
//  * automatic page libs and footer management;
 ...
 * <li>automatic page libs and footer management;</li>
...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		...
		/**
		 * @var Minimum distance between libs and top page margin.
		 * @access protected
		 */
		...
		/**
		 * @var String to print as title on document libs.
		 * @access protected
		 */
		protected $header_title = '';

		/**
		 * @var String to print on document libs.
		 * @access protected
		 */
		...
		/**
		 * Table libs content to be repeated on each new page
		 * @access protected
		...
		/**
	 	 * Set libs data.
		 * @param string $ln libs image logo
		 * @param string $lw libs image logo width in mm
		 * @param string $ht string to print as title on document libs
		 * @param string $hs string to print on document libs
		 * @access public
		 */
		...
		/**
	 	 * Returns libs data:
	 	 * <ul><li>$ret['logo'] = logo image</li><li>$ret['logo_width'] = width of the image logo in user units</li><li>$ret['title'] = libs title</li><li>$ret['string'] = libs description string</li></ul>
		 * @return array()
		 * @access public
		 * @since 4.0.012 (2008-07-24)
		 */
		...
		/**
	 	 * Set libs margin.
		 * (minimum distance between libs and top page margin)
		...
		/**
	 	 * Returns libs margin in user units.
		 * @return float
		 * @since 4.0.012 (2008-07-24)
		 * @access public
		 */
		...
		/**
	 	 * Set a flag to print page libs.
		 * @param boolean $val set to true to print the page libs (default), false otherwise.
		 * @access public
		 */
		...
		/**
	 	 * This method is used to render the page libs.
		...
		/**
	 	 * This method is used to render the table libs on new page (if any).
	 	 * @access protected
		...
		public function Output($name='doc.pdf', $dest='I') {
			...
			switch($dest) {
				...
				case 'D': {
					...
					// use the Content-Disposition libs to supply a recommended filename
			...
			foreach ($this->FontFiles as $file => $info) {
				...
				if (!$this->empty_string($fontfile)) {
					...
					if ((!$compressed) AND (isset($info['length2']))) {
						...
						if ($header) {
							//Strip first binary libs
							$font = substr($font, 6);
						}
						if ($header AND (ord($font{$info['length1']}) == 128)) {
							//Strip second binary libs
		...
		public function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false) {
			...
			// strip binary bytes in front of PS-libs
		...
		protected function getHtmlDomArray($html) {
			...
			while ($elkey < $maxel) {
				...
				if (preg_match($tagpattern, $element)) {
					...
					// check if we are inside a table libs
					if ($tagname == 'thead') {
						if ($element{0} == '/') {
							$thead = false;
						} else {
					...
					if ($element{0} == '/') {
						...
						// store libs rows on a new table
