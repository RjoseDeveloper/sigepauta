...
function register_utilizador(ctr){


    var user = $('#username').val();
    var pass = $('#password').val();
    var nomec = $('#fullname').val();
    var prev = $('#previlegio').val();
    var sexo = $('#sexo').val();
    var textPrev = $('#previlegio option:selected').text();

    var url ='';

    if (ctr === 0){

        url = '../requestCtr/Processa_utilizador.php?acao=1';
    }else{
        var iduser = $('#iduser').val();
        url = '../requests/UserRequests.php?acao=2&iduser='+iduser;
    }
    ...
    $.ajax({
        ...
        data:{sexo:sexo, username:user, password:pass,
            fullname:nomec, previlegio:prev},

        success:function(sms){
            $('.sms_report').html(sms); //.fadeOut(5000);

            if (textPrev !='Estudante'){
